"""
Jane — SAFE Consumer Interface Server
======================================
Jane's own FastAPI server at port 8421 (assigned by Willow).
Proxies chat to Willow's agent API, journals all exchanges.
Normal users interact with Willow's system through Jane — never directly.
"""
...
app = FastAPI(title="Jane", description="SAFE Consumer Interface — Willow")
...
async def chat(req: ChatRequest):
    """Chat with Jane. Proxies to Willow agent API and journals both sides."""
    async with httpx.AsyncClient(timeout=60.0) as client:
        # Call Willow's Jane agent
        r = await client.post(
            f"{WILLOW_URL}/api/agents/chat/jane",
            json={
                "message": req.message,
...
if __name__ == "__main__":
    print(f"Jane starting on port {JANE_PORT}...")
    uvicorn.run(app, host="0.0.0.0", port=JANE_PORT, log_level="info")
