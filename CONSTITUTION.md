...
**Field evidence (2026-08-06), on automation that assists a reserved judgment without deciding it — not adopted, named so it is not re-derived from scratch.** Drift detection is this constitution's one existing case of automated signal (Auto-Applied) feeding a consequence for a reserved actor (suspension, escalating to Operator Key). The same shape recurs wherever an automated check sits next to a human-reserved classification it cannot itself make — e.g., a pattern matcher advising on a data-sensitivity rung a human must still assign. Reviewing one such design (`homestead/docs/PLAN-first-runnable.md`, proposing a PII-pattern matcher advisory to a human classification decision, unbuilt) surfaced three conditions that recurred independently of that design and may generalize to any Auto-Applied check touching a reserved decision: **(1)** it may only prompt the reserved decision-maker toward the more restrictive answer, never argue them toward the less restrictive one, since no automated check may itself perform what would otherwise be a lowering of protection (the shape of §0.3 and I-11's "absence fails closed," applied to advice rather than to data); **(2)** it is advisory only and never itself the gate, or a human judgement has been relocated into a pattern list without a decision to relocate it; **(3)** its silence must never be read as clearance — the absence of a flag is not evidence of absence, so "the check found nothing" and "the check verified there is nothing" must remain visibly different states to whoever reads the output. Read as evidence, not doctrine, per the standard Article VII's own field-evidence note sets: it names a candidate general principle, it does not adopt one, and it is not this document's article to place until an operator decides whether it belongs under Article I, Article II, or a new provision.
...
**VI.2a — Sequence is derived from the chain, not asserted by a column.** *(proposed 2026-08-06, unratified — see Amendment History)* An implementation of the Record MUST establish the order of entries from the hash chain's own linkage — each entry naming the hash of the one before it — and MUST NOT rely on an unsigned timestamp or insertion-order column to establish that order for verification purposes. Such a column, where one exists for indexing or display, is metadata, not evidence: altering it is not a VI.2 "ordering repair" unless it is itself a recorded, human-authorized act, and an implementation that lets it be altered by an ordinary write has not made ordering tamper-evident even though it made content tamper-evident. Verification MUST walk the chain by its own links, from genesis, rather than by sorting a column nothing signs. *(Motivating case: found independently, in two separate implementations of FRANK, on the same day — `willow-mcp/src/willow_mcp/governance_ledger.py` and `willow-2.0/core/sqlite_bridge.py` + `core/pg_bridge.py` — both establishing `frank_ledger`'s walk order by `ORDER BY created_at`, a column absent from what either implementation signs. See `willow-mcp#280`, `willow-mcp#283`, `willow-2.0#888`. Two independent implementations reaching the same gap is what the Preamble asks this document to learn from before, not after, a third one does.)*
...
| Establish entry sequence | Auto-Applied | By hash-chain linkage from genesis, never by an unsigned column (VI.2a) |
...
- **Every required compliance test SHALL be non-vacuous in both directions.** *(proposed 2026-08-06, unratified)* It must prove the forbidden act is refused, **and** it must prove — by a check kept separate from the adversarial verdict, so a passing verdict never mixes "refused the bad case" with "granted the good one" — that a legitimately authorized act of the same kind is granted. A gate proven only to refuse has not been shown to discriminate; it may simply be closed, which is not the invariant this Appendix asks to be held. *(Cited as evidence for this requirement rather than proposed as invention: the pattern already exists, independently authored, in at least three places — `willow-2.0/constitution/compliance.py`'s liveness anchors, run by CI separately from the adversarial `Verdict`; `safe-app-store/stores/decisions/fleet.json`'s covenant tests, which prove both that the real record is clean and that each rejection check is reachable; and `homestead`'s corpus-completeness guards. None of the three cites the others. A requirement three teams independently reinvented is a requirement this Appendix should state once.)*
- **The set of clauses this Appendix requires coverage for SHALL itself be checked against the set of registered compliance tests**, and an uncovered clause SHALL be reported as a breach of this Appendix, not left to be found by manual reading. *(proposed 2026-08-06, unratified. Motivating case: on 2026-08-06, two of the six Eternity Clauses — §0.1 and §0.6 — were found to have no registered probe in `willow-2.0/constitution/cases/`, undetected by the suite itself, because `run_compliance.py`'s registry and its own tests assert only over the probes that exist, never over the clause list this document names. See `willow-2.0#887`. Appendix A already states "No orphan authority. No orphan enforcement" — this bullet is that sentence, made checkable rather than merely written.)*
...
| 2026-08-06 | VI, App. B | Draft 0.7 — AI cross-repo audit pass (Claude, from `homestead`/`safe-app-store`/`willow-mcp`/`willow-2.0`; single-model, not an Independent Witness per this document's own IV definition — proposed only): VI.2a added (sequence must be chain-derived, never column-derived), evidenced by the identical `frank_ledger` ordering gap independently found in two separate FRANK implementations the same day (`willow-mcp#280`/`#283`, `willow-2.0#888`); two Appendix B bullets added — non-vacuous-in-both-directions required of every compliance test (evidenced by the same discipline already independently present, uncited by each other, in `willow-2.0`'s compliance package, `safe-app-store`'s fleet-decision covenant tests, and `homestead`'s corpus guards), and clause-coverage completeness itself made a checkable requirement (evidenced by §0.1/§0.6 having zero registered probes despite Appendix B's SHALL, found by manual reading — `willow-2.0#887`) | *unratified draft* |
| 2026-08-06 | I | Draft 0.7 — Field Evidence note added to I.4: three conditions for automation that advises but does not decide a reserved judgment (raise-only, advisory-only, silence ≠ clearance), drawn from reviewing an unbuilt PII-matcher design in `homestead`; explicitly not adopted as doctrine, in the same shape as the 2026-07-07 Article VII note | *unratified draft* |
