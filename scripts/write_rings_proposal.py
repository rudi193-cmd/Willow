...
**Commit ID:** add-core-rings-module-2026-02-25

---

## Summary

boot.py established the boot contract -- port, PID, config. rings.py establishes the
...
A fresh Willow node starts as source-ring only. Bridge ring activates when the first
peer enrolls. Continuity ring activates only when gate.py is explicitly present -- a
peer node without gate.py cannot join the continuity ring. Every outbound pigeon is
packaged by make_pigeon() with content + gate_conditions + SEED_PACKET. Every inbound
pigeon is validated by validate_inbound() -- missing gate_conditions = rejected.

Stdlib-only. Under 150 lines. No side effects on import.

## Proposed Changes

**File:** `core/rings.py` (NEW)

## Rationale

- Paper 13: "The canon travels. The gate does not -- unless you make it travel."
  rings.py makes it travel.
- A fresh node knows it runs source ring. It does not assume bridge or continuity.
- enroll_peer() activates bridge ring on first peer -- the flock grows itself.
- enroll_gate() requires gate.py present locally -- you cannot proxy governance
  you do not have.
- make_pigeon() packages gate_conditions into every outbound payload.
- validate_inbound() rejects peers that stripped their gate.
- ring_status() exposes node ring state to server.py and dashboard.

## Risk Assessment

- **Risk Level:** LOW
- **Reversible:** YES -- new file only, no existing module is modified
- **Dependencies:** Imports from core.boot (load_config, _config_to_dict, CONFIG_PATH).
  boot.py must be present.
- **Testing:** Import core.rings in REPL. Call ring_status() (expect source=True,
  bridge=False, continuity=False). Call enroll_peer('test-node-8420'), verify
  bridge=True. Call enroll_gate(), verify result based on gate.py presence. Call
  make_pigeon({}, {}) and verify all four fields. Call validate_inbound({}) and
  verify rejection.

## Delta-E Impact

Expected Delta-E: +0.028 (closes the ungoverned pigeon gap -- every outbound message
now carries gate conditions, making the gate a traveler not a checkpoint)
...
+Version: 1.0.0
+Status: Active
+Last Updated: 2026-02-25T05:31:00Z
+Checksum: DS=42
+
+Responsibilities:
